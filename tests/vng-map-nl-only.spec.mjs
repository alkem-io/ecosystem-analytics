 ...
 *
 * The VNG dashboard map MUST render ONLY the Netherlands: real CARTO map-tile detail
 ...
 *
 * This test reproduces that exact layering (a RED layer stands in for the tiles) and
 * asserts, by sampling real rendered pixels, BOTH statically AND under a zoom-like
 * transform, that:
...
test('VNG map shows tiles INSIDE NL and nothing OUTSIDE — static (constitution §VII / FR-048)', async ({
  page,
}) => {
...
test('VNG map stays Netherlands-only UNDER ZOOM (the overlay tracks the zoom transform)', async ({
  page,
}) => {
  const { geoMercator, geoPath } = await import(
    resolve(REPO, 'frontend/vng/node_modules/d3-geo/src/index.js')
