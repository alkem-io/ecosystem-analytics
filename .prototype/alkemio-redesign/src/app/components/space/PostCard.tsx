...
import { MessageSquare, MoreHorizontal, Share2, ThumbsUp, LayoutGrid, FileText, Presentation, Maximize2 } from "lucide-react";
    ...
    <Card className="group hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 border-border/60">
      ...
      <CardContent className="px-6 pb-3">
        ...
        {post.type === "whiteboard" && post.contentPreview?.imageUrl && (
          <div className="rounded-lg overflow-hidden border border-border bg-muted/30 relative aspect-video">
            ...
            />
            <div className="absolute inset-0 flex items-center justify-center bg-[#1D384A]/10 group-hover:bg-[#1D384A]/20 transition-colors">
            ...
            {post.contentPreview.whiteboards.slice(0, 4).map((wb, idx) => {
              ...
              return (
                <div key={idx} className="group/wb relative rounded-lg overflow-hidden border border-border bg-muted/30 aspect-[4/3] cursor-pointer hover:ring-2 hover:ring-primary/50 transition-all">
                  ...
                  {!showMoreOverlay && (
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/wb:opacity-100 transition-opacity duration-200 bg-[#1D384A]/40">
                  ...
                  {!showMoreOverlay && (
                    <div className="absolute inset-0 bg-gradient-to-t from-[#1D384A]/80 via-[#1D384A]/20 to-transparent p-3 flex flex-col justify-end pointer-events-none">
                  ...
                  {showMoreOverlay && (
                    <div className="absolute inset-0 flex items-center justify-center bg-[#1D384A]/60 backdrop-blur-[2px]">
      ...
      <CardFooter className="px-6 py-3 border-t bg-muted/5 flex items-center gap-4">
        <Button variant="ghost" size="sm" className="h-8 gap-2 text-muted-foreground hover:text-foreground pl-0 hover:bg-transparent">
          ...
          <span className="text-xs">{post.stats.likes} Likes</span>
        </Button>
        <Button variant="ghost" size="sm" className="h-8 gap-2 text-muted-foreground hover:text-foreground hover:bg-transparent">
