 ...
 * browser when `credentials: 'include'` is used), calls Kratos's
 * `/sessions/whoami` endpoint to validate the session, and returns the
 * user's identity and session token.
 *
...
export async function ssoDetectHandler(req: Request, res: Response) {
  ...
  try {
    ...
    // If we got user identity but no token from whoami, we can try to use
    // the Kratos session token exchange (toSession endpoint)
    if (!response.token) {
      // Try the /sessions/token-exchange if available, otherwise fall back
      // to using the cookie as-is (some Kratos versions expose session_token)
