  ...
  if (mapMode) {
    ...
    nodes.forEach(node => {
      if (node.location) {
        ...
        const cy = height / 2;
        positions[node.id] = {
          x: cx + (node.location.lng - 10) * (scale / 10),
          y: cy - (node.location.lat - 50) * (scale / 10)
        };
      } else {
        unlocated.push(node);
      }
    ...
    unlocated.forEach((node, i) => {
      ...
      const r = 30 + i * 8;
      positions[node.id] = { x: ox + Math.cos(a) * r, y: oy + Math.sin(a) * r };
    });
    return positions;
  }
    ...
    >
        ...
        {mapMode && (
          <div className="absolute inset-0 pointer-events-none" style={{ opacity: 0.2 }}>
            <svg width="100%" height="100%" viewBox="0 0 800 600" preserveAspectRatio="xMidYMid slice">
              <path d="M150,180 Q250,120 400,160 T700,140 L750,300 Q600,450 400,400 T150,500 Z" fill="var(--muted)" />
              <path d="M50,100 Q120,80 140,150 T50,250 Z" fill="var(--muted)" />
              <rect x="20" y="420" width="200" height="160" rx="20" fill="none" stroke="var(--muted-foreground)" strokeDasharray="4 4" opacity="0.5" />
              <text x="40" y="445" style={{ fontSize: '11px', fill: 'var(--muted-foreground)', fontFamily: "'Inter', sans-serif", opacity: 0.5 }}>UNLOCATED</text>
            </svg>
          </div>
        )}
      ...
      {mapMode && (
        ...
        >
          ...
          <span>Map view — most entities lack geolocation</span>
        </div>
      )}
