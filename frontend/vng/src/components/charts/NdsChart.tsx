...
/**
 * Bar chart of counts by NDS category (US3, FR-021). Category labels are localised
 * by key via i18n (`categories.nds.<key>`) with a fallback to the raw key (FR-037).
 */
export function NdsChart({ dimension, source }: NdsChartProps) {
  ...
  const data = categories.map((c) => ({
    key: c.key,
    label: t(`categories.nds.${c.key}`, { defaultValue: c.key }),
    count: c.count,
  }));
  ...
  return (
    <section className="flex flex-col gap-1 rounded-lg border border-border bg-card p-4">
      <h3 className="text-sm font-semibold text-foreground">{t('dashboard.nds')}</h3>
